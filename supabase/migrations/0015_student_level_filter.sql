-- Student-level filter for explore_courses and rank_classes.
--
-- Both gain an optional `p_student_level text` argument that filters on the
-- section's student level (sections.student_level, values 'Undergraduate',
-- 'Graduate', 'Both'). The matching is audience-inclusive: a cross-listed
-- 'Both' section counts toward either population, so
--   'Undergraduate' -> student_level in ('Undergraduate','Both')
--   'Graduate'       -> student_level in ('Graduate','Both')
--   null             -> no filter (the UI's "Both" option, i.e. all levels)
-- The UI defaults the control to Undergraduate.
--
...
-- ---------------------------------------------------------------------------
-- explore_courses  (+ p_student_level)
-- ---------------------------------------------------------------------------
drop function if exists explore_courses(text, text, text, text, int, numeric, int, text, text, int, int);
...
-- ---------------------------------------------------------------------------
-- rank_classes  (+ p_student_level)
-- ---------------------------------------------------------------------------
drop function if exists rank_classes(text, text, int, int, int, int);
